...
db_size=$(curl -s "$url/metrics" | grep -v "^#" | grep "$db_size_metric" | awk '{print $2}')
...
echo "`date "+%Y-%d-%mT%H:%M:%S"` -- used: $current_size, db_size_limit: $db_size, percentage used: $used%"
if [ $used -gt 80 ]; then
